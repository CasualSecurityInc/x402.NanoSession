...
NanoSession Rev5 avoids this accidental self-invalidation entirely by mandating that the **client must broadcast the block to the network first**, relying on **Raw Tagging** to identify the transaction, and only bringing the *confirmed receipt* (the block hash) to the server.
