...
**Base Specification:** `x402_NanoSession_rev5_Protocol.md`
...
3. **Queue Complexity:** Facilitators must ensure asynchronous task idempotency—if the server crashes while processing a rebate queue, it must not accidentally refund the same dust twice upon rebooting.
