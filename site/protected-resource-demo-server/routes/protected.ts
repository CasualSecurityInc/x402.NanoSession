...
import { encodePaymentRequired, decodePaymentSignature, PaymentPayload } from '@nanosession/core';
...
protectedRoute.get('/', async (req: Request, res: Response) => {
    ...
    const tagMultiplier = BigInt(reqs.extra.nanoSession.tagMultiplier || '1');
    const taggedAmountRaw = (BigInt(reqs.amount) + BigInt(reqs.extra.nanoSession.tag) * tagMultiplier).toString();
