...
import { encodePaymentRequired, decodePaymentSignature, PaymentPayload, calculateTaggedAmount } from '@nanosession/core';
...
async function settleAndRespond(
    ...
    try {
        const result = await facilitator.handleSettle(requirements, {
            x402Version: 2,
...
protectedRoute.get('/', async (req: Request, res: Response) => {
    ...
    const envModulus = process.env.NANO_TAG_MODULUS || process.env.TAG_MODULUS;
    const envMultiplier = process.env.NANO_TAG_MULTIPLIER || process.env.TAG_MULTIPLIER;
    const reqs = facilitator.getRequirements({
        amount: amountToCharge,
        payTo: payToAddress,
        maxTimeoutSeconds: 180, // 3 minute timeout
        tagModulus: envModulus ? parseInt(envModulus, 10) : 10000,
        tagMultiplier: envMultiplier
    });
    ...
    // Use centralized calculation utility for tagged amount
    ...
    // Construct the PaymentRequired payload
...
protectedRoute.post('/', async (req: Request, res: Response) => {
    ...
    try {
        ...
        res.status(400).json({ error: "Invalid paymentSignature format" });
        return;
    }
